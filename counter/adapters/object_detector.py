...
from domain.ports import ObjectDetector
...
class FakeObjectDetector(ObjectDetector):
    """
    A fake object detector for testing purposes that always returns the same prediction.

    """

    def predict(self, image: BinaryIO) -> List[Prediction]:
        try:
            c_name = random.choice(('dog', 'cat', 'rat', "mat", "rabbit"))
            return [Prediction(class_name=c_name,
                               score=0.999190748,
            ...
            logger.exception(f"Exception occurred in FakeObjectDetector: {e}")
...
class TFSObjectDetector(ObjectDetector):
    ...
    def predict(self, image: BinaryIO) -> List[Prediction]:
        try:
            ...
            predict_request = '{"instances" : %s}' % np.expand_dims(
                np_image, 0).tolist()
            ...
            logger.exception(
                f"Error occurred while predicting with TFSObjectDetector: {e}")
            return e
    ...
    def __raw_predictions_to_domain(self, raw_predictions: dict) -> List[Prediction]:
        try:
            ...
            for i in range(0, num_detections):
                ...
                box = Box(xmin=detection_box[1], ymin=detection_box[0],
                ...
                predictions.append(Prediction(
                    class_name=class_name, score=detection_score, box=box))
            return predictions
        except Exception as e:
            logger.exception(
                f"Error occurred while processing predictions with TFSObjectDetector: {e}")
            return e
